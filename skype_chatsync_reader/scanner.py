    ...
    def parse(self):
        ...
        for msg in self.scanner.blocks[2].block_data:
            ...
            try:
                ...
                self.conversation.append(ConversationMessage(msg.header.timestamp, participant1 if user_id == user_id1 else participant2, unicode(msg_text, 'utf-8'), is_edit))
            except Exception, e:
                warnings.warn("Exception: %s" % e.message)
                self.errors += 1
...
def parse_chatsync_profile_dir(dirname):
    ...
    for f in files:
        try:
            data = parse_chatsync_file(f)
            # for tmp in data:
            #     if tmp.author == "simdephoguom":
            #         print ">>>>>>>",f
